...
Pull it from DockerHub (or another registry)

Start a container from it in a pod
...
You apply a YAML file (kubectl apply -f selenium-tests.yaml)

K8s scheduler chooses a worker node

Kubelet on that node pulls the image

It creates a Pod with your container

Runs the CMD/ENTRYPOINT from the Dockerfile (e.g. mvn test)

Kubernetes monitors the container (restart or clean up based on type)
